...
private:
    ...
    void createCommandBuffers(){
        ...
        for (size_t i = 0; i < commandBuffers.size(); i++) {
            ...
            vkCmdDrawIndexed(commandBuffers[i], static_cast<uint32_t>(node.getInd().size()), 1, 0, 0, 0);
    ...
    void createVertexBuffer(){
        ...
        VkDeviceSize bufferSize = sizeof(node.getVert()[0]) * node.getVert().size();
        ...
        memcpy(data, node.getVert().data(), (size_t) bufferSize);
    ...
    void createIndexBuffer(){
        VkDeviceSize bufferSize = sizeof(node.getInd()[0]) * node.getInd().size();
        ...
        void* data;
        vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
        memcpy(data, node.getInd().data(), (size_t) bufferSize);
        vkUnmapMemory(device, stagingBufferMemory);
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.proj[1][1] *= -1;
        ubo.MinvT = glm::transpose(glm::inverse(ubo.model));
